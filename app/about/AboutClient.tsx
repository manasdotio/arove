...
export default function AboutClient() {
  ...
  const toggleFaq = (index: number) => setOpenFaqIndex((prev) => (prev === index ? null : index));
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <div>
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-4 sm:gap-x-8">
              {[
                { value: '172+', label: 'Digital projects' },
                { value: '500m', label: 'Client Revenue Generated', sup: true },
                { value: '100%', label: 'Client Satisfaction' },
                { value: '612+', label: 'Cups of Coffee' },
              ].map((stat) => (
                <div key={stat.label} className="group">
                  <div className="text-4xl md:text-5xl font-display font-bold text-text-main mb-1.5 group-hover:text-primary transition-colors duration-300">
                    {stat.value}
                  </div>
                  <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-on-surface/50">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full flex items-center justify-between text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:rounded-2xl cursor-pointer"
